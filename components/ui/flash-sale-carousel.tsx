      ...
      <div className="overflow-hidden px-4 relative z-10" ref={emblaRef}>
        <div className="flex gap-3">
          {items.map((item) => (
            <div
              key={item.id}
              className="flex-[0_0_160px] sm:flex-[0_0_180px] md:flex-[0_0_200px] bg-white rounded-xl overflow-hidden cursor-pointer group shadow-sm flex flex-col justify-between"
            ...
            >
              ...
              <div className="w-full relative overflow-hidden bg-[#00c5a2] flex items-center h-[52px]">
                 ...
                 <div className="absolute inset-y-0 left-0 w-[70%] bg-[#e6fbf7] flex items-center px-3" style={{ clipPath: 'polygon(0 0, 100% 0, 80% 100%, 0 100%)' }}>
                   <span className="text-[#00c5a2] font-bold text-lg md:text-xl">৳{item.price}</span>
                 </div>
              </div>
            </div>
          ))}
